...
def yuv_inter(sr,sdr):
    # print(sr.shape,sdr.shape)
    sdr=sdr.astype(np.uint16)
    sdr=cv2.resize(sdr,(sdr.shape[1]*2,sdr.shape[0]*2))
    sdr=sdr*256

    b_, g_, r_ = sdr[:, :, 0], sdr[:, :, 1], sdr[:, :, 2]
    y_ = 0.2627 * r_ + 0.6780 * g_ + 0.0593 * b_
    Cb_ = (b_ - y_) / 1.8814
    Cr_ = (r_ - y_) / 1.4746

    b, g, r = sr[:, :, 0], sr[:, :, 1], sr[:, :, 2]
    y = 0.2627 * r + 0.6780 * g + 0.0593 * b
    Cb = (b - y) / 1.8814
    Cr = (r - y) / 1.4746

    factor_sr = 0.8 #(1-0.5678)
    factor_sdr = 0.2 #0.5678

    sr[:, :, 0] = 1.8814 * (Cb * factor_sr + Cb_ * factor_sdr) + y  # b
    sr[:, :, 2] = 1.4746 * (Cr * factor_sr + Cr_ * factor_sdr) + y  # r
    # sr[:, :, 1] = (y - 0.2627 * r - 0.0593 * b) / 0.6780  # g
    sr[:, :, 1] = (y - 0.2627 * sr[:, :, 2] - 0.0593 * sr[:, :, 0]) / 0.6780  # g
    sr = np.clip(sr, 0, 65535)
    return sr
...
def save_img(img,img_path,img_type=np.uint16):
    img=np.clip(img,0,65535)
    cv2.imwrite(img_path, img.astype(img_type))


def save_img_test(img,sdr,img_path,img_type=np.uint16):
    img=np.clip(img,0,65535)
    sdr=np.clip(sdr,0,255)
    img2=yuv_inter(img,sdr)
    cv2.imwrite(img_path, img2.astype(img_type))
